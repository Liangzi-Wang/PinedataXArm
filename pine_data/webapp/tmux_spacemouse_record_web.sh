...
XARM_USE_TOOL_TWIST_AA="${XARM_USE_TOOL_TWIST_AA:-1}"
...
printf -v robot_env_prefix \
  'export ROBOT_BACKEND=%q ROBOT_IP=%q XARM_IP=%q XARM_CONTROLLER_PATH=%q XARM_TELEOP_SPEED=%q XARM_TELEOP_ANGULAR_SPEED=%q XARM_MOVE_ACCELERATION=%q XARM_COMMAND_PERIOD_S=%q XARM_TELEOP_CONTROL_MODE=%q XARM_COMMAND_TRANSLATION_MAP=%q XARM_USE_TOOL_TWIST_AA=%q XARM_TOOL_TWIST_AXIS=%q SPACEMOUSE_RESPONSE_EXPONENT=%q; ' \
  "$ROBOT_BACKEND" \
  "$ROBOT_IP" \
  "$ROBOT_IP" \
