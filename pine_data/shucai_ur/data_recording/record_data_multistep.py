...
## Press 'c' to start recording new episode & step 1
## Press 'n' to stop recording 1 & start recording step 2
## Press 's' to stop recording entire episode 
...
class Data(): 
    ...
    def get_point_cloud(self, capture):
        if capture is not None and capture.color is not None and capture.depth is not None:
            ...
            # print("Final point cloud shapemohan:", points.shape)
            return points
        else:
    ...
    def on_press(self, key):
        global recording, current_time, current_episode_folder, current_step_folder, current_step_index
        ...
        try:
            if key.char == 'c':
                if not recording:
                    ...
                    current_step_folder = os.path.join(current_episode_folder, f"step{current_step_index}")
                    os.makedirs(current_step_folder, exist_ok=True)

                    recording = True
                    print(f"Started recording episode {current_time}, step{current_step_index}...")

            elif key.char == 'n':
                if recording:
                    # Save current step
                    print(f"Saving step{current_step_index} data...")
                    self.save_data(current_step_folder)

                    # Create next step
                    current_step_index += 1
                    current_step_folder = os.path.join(current_episode_folder, f"step{current_step_index}")
                    os.makedirs(current_step_folder, exist_ok=True)
                    print(f"Started step{current_step_index}...")

            elif key.char == 's':
                if recording:
                    print(f"Stopping recording, saving step{current_step_index}...")
                    self.save_data(current_step_folder)
                    
                    recording = False
                    print(f"Episode {current_time} recording stopped.")
                    time.sleep(1)
                ...
                if not recording:
                    ...
                    if 'y' in confirmation and current_episode_folder:
            ...
            if recording:
                ...
                if joint_state is not None and eef_state is not None:
                    ...
                    self.synchronize_data(point_cloud, color_image, depth_image, color_hand_image, depth_hand_image, point_cloud_hand, point_concatenate, \
                                          joint_state, joint_action, eef_state, eef_action, eef_force, save_path=current_step_folder)
                    print("saved")
    ...
    def synchronize_data(self, point_cloud, color_image, depth_image, color_hand_image, depth_hand_image,point_cloud_hand, point_concatenate, joint_state, joint_action, eef_state, eef_action, eef_force, save_path):
        # visual observations
    ...
    def save_data(self, step_folder):
        # Define file paths within the step folder
        color_file = os.path.join(step_folder, "rgb.npy")
        depth_file = os.path.join(step_folder, "depth.npy")
        point_cloud_file = os.path.join(step_folder, "point_cloud.npy")
        point_cloud_hand_file = os.path.join(step_folder, "point_cloud_hand.npy")
        point_concatenate_file = os.path.join(step_folder, "point_concatenate.npy")

        color_hand_file = os.path.join(step_folder, "rgb_hand.npy")
        depth_hand_file = os.path.join(step_folder, "depth_hand.npy")

        joint_state_file = os.path.join(step_folder, "joint_state.npy")
        joint_action_file = os.path.join(step_folder, "joint_action.npy")
        eef_state_file = os.path.join(step_folder, "eef_state.npy")
        eef_action_file = os.path.join(step_folder, "eef_action.npy")
        eef_force_file = os.path.join(step_folder, "eef_force.npy")
...
def main():
    ...
    global data_folder, recording, instruction, current_episode_folder, current_step_folder, current_step_index
    # data_folder = "/home/mainuser/UR5_Policy/data_recording/pick_ball"
    # data_folder = "/media/mainuser/Vision 1/UR5_Policy/data"
    # data_folder ="/home/mainuser/UR5_Policy/data_recording/shuffle_cube"
    # data_folder ="/home/mainuser/UR5_Policy/data_recording/mobilecube"
    # data_folder = "/media/mainuser/a6300fe1-151f-4e9e-8790-c4826f4ee765/home/mainuser/Mohan_Optical_Flow/ball2box"
    # data_folder = "/media/mainuser/a6300fe1-151f-4e9e-8790-c4826f4ee765/data_recording/data_weighing_scale_YuanYao/weighing_scale_flexibleObject"
    # data_folder = "/media/mainuser/a6300fe1-151f-4e9e-8790-c4826f4ee765/data_recording/plugnplay_YinYu/force_plug_push"
    ...
    data_folder = "/media/mainuser/a6300fe1-151f-4e9e-8790-c4826f4ee765/data_recording/assembly"
    # data_folder = "/media/mainuser/MohanSSD/data/box_on_box"
    # data_folder = "/home/mainuser/UR5_Policy/data_recording/test"

    recording = False
    instruction = input("Type the instruction of this recording capture: ") # not used currently
    
    #for multistep recording
    current_episode_folder = None
    current_step_folder = None
...
# todo:
# need to clear buffer before recording (currently deleted data gets added to next recording)
# need to break data when going to next step (currently just add on ie step2 = step1 + step2)