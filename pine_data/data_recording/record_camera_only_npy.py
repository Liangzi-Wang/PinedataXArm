...
class CameraRecorder(): 
    ...
    def save_data(self, episode_folder):
        ...
        rgb_file = os.path.join(episode_folder, "rgb_hand.npy")
        depth_file = os.path.join(episode_folder, "depth_hand.npy")
        timestamp_file = os.path.join(episode_folder, "timestamps.npy")
        
        np.save(rgb_file, color_hand_image)
        np.save(depth_file, depth_hand_image)
        ...
        metadata = {
            ...
            'camera': 'D405_hand',
            'frequency': 15,
            'start_time': float(timestamps[0]) if len(timestamps) > 0 else 0.0,
            'end_time': float(timestamps[-1]) if len(timestamps) > 0 else 0.0,
        ...
        print(f"Saved {len(color_hand_array)} frames:")
...
def main():
    ...
    base_data_folder = os.path.expanduser("~/pine_data/recordings")
    data_folder = os.path.join(base_data_folder, instruction, "camera_npy")
    ...
    print("\n" + "="*50)
    print("SINGLE CAMERA RECORDING SYSTEM")
    print("D405 (Hand)")
    print("="*50)
    print("\nControls:")
    ...
    print("\nStopped single camera recording system.")
